...
void gnc_update_sx_pending_ui(void) {
	...
	static action_toolbar_dynamic_labels act_dyn_toolbar_labels[] = {
	  { "TBActionsSinceLastRunAction", NULL },
	  { NULL, NULL },
	};
	...
	pending_sx_count = sx_sincelast_checkpend();
	pending_sx_count = abs(pending_sx_count);

	if (act_dyn_toolbar_labels[0].label != NULL) {
	   g_free(act_dyn_toolbar_labels[0].label);
	}

	if (pending_sx_count)
		act_dyn_toolbar_labels[0].label = g_strdup_printf(N_("%d Pending"),pending_sx_count);
	else
		act_dyn_toolbar_labels[0].label = g_strdup(N_("0 Pending"));
	...
	for (walker = active_windows; walker; walker = g_list_next(walker)) {
	    ...
	    if (action_group != NULL) {
	        gnc_plugin_init_short_names (action_group,
					    (action_toolbar_labels *) act_dyn_toolbar_labels);
...
gint sx_sincelast_checkpend(void)
{
	...
	DEBUG( "sx-sincelast-checkpend %d.", summary.num_instances);
    return summary.num_instances;
